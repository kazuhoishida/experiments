...
const FooterNavigation = ({nav}: Props) => {
  ...
  return nav && (
    <Provider>
      <div className={`fixed bottom-[2.5vh] left-1/2 -translate-x-1/2 z-30 duration-[400ms] ${pathname === '/' ? 'opacity-100' : isIntersected ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <ul
          className={`
