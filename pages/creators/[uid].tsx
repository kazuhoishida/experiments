          ...
          <Bounded className="relative px-4 pb-0 md:mx-auto md:px-[10vw] md:pb-20 [&>div]:max-w-none md:[&>div]:flex md:[&>div]:gap-x-[4vw]">
            ...
            <div className="mb-8 h-full items-center gap-x-4 md:block md:w-1/2">
              ...
              <span className="aspect-square inline-block h-10 w-10 overflow-hidden rounded-full md:h-[4vw] md:w-[4vw]">
                {creator.data?.face?.url && <FutureImage src={creator.data?.face?.url} alt={creator.data?.face?.alt ?? ""} className="h-full w-full object-cover" />}
              </span>
            </div>
            <div className="md:w-1/2 md:pt-10">
              <div className="mb-8 flex gap-x-4 text-sm font-bold">
                {github && (
                  <PrismicLink field={github} className="flex h-6 w-24 place-content-center place-items-center rounded-md border border-v-red font-flex text-v-red md:hover:bg-v-red md:hover:text-white">
                    GitHub
                  </PrismicLink>
                )}
          ...
          {projects.slice(0, 8).map(
            (project, index) =>
              prismicH.isFilled.linkToMedia(project.data?.featuredMedia) && (
                <PrismicLink field={prismicH.documentToLinkField(project)} key={project.id} className="group relative h-full border-black md:border-l">
                  <div className="flex w-full shrink-0 flex-col gap-y-4 py-8 duration-[200ms] md:h-full md:justify-end md:pb-24 md:group-hover:-translate-y-3">
                    <div className="px-4">
                      <span className="absolute top-0 right-4 text-[48px] tracking-tighter text-white [-webkit-text-stroke:1px_black] md:-top-6 md:-right-6 md:text-[4vw]">{String(index + 1).padStart(2, "0")}</span>
                    ...
                    <FutureImage src={project.data?.featuredMedia.url ?? ""} alt={project.data?.title ?? "PROJECT"} className="aspect-[5/3] w-[90vw] object-cover shadow duration-[600ms] md:group-hover:shadow-lg" />
                  </div>
                </PrismicLink>
              )
          )}
