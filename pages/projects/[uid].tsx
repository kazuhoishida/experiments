  ...
  return (
    <div className="flex place-content-center">
      <button className="group flex place-items-center gap-x-6" onClick={back}>
        ...
        <nav className="font-bold-h1 font-flex text-3xl font-[640] duration-[500ms] group-hover:-translate-x-4">Go Back</nav>
      </button>
    </div>
  )
}
  ...
  return (
    <Layout nav={nav}>
      ...
      <main>
          ...
          <div className="px-4 md:px-0">
            <div className="md:mb-[max(10vh,30px)] md:flex md:px-[5vw]">
              ...
              <div className="flex place-items-center gap-x-4">
                {face && (
                  <PrismicLink className="relative flex aspect-1 h-12 w-12 place-items-center gap-x-2 font-flex font-[640] [font-stretch:32%]" document={creator}>
                    <Image className="rounded-full" src={face} alt={(creator.data?.face?.alt || creator.data.name) ?? "CREATOR"} fill />
                  </PrismicLink>
                )}
                <div className="flex flex-col">
                  <PrismicLink className="flex place-items-center gap-x-2 whitespace-nowrap font-flex font-[640] [font-stretch:32%]" document={creator}>
                    {creator.data.name}
                  </PrismicLink>
                  <p className="whitespace-nowrap font-flex font-[640] [font-stretch:32%]">{dateFormatter.format(date!)}</p>
            ...
            <div className="md:flex">
              <div className="md:w-2/5 md:pl-[5vw] md:pr-10">
                <div className="flex gap-x-6 py-6 empty:!py-0">
                  {demo && (
                    <PrismicLink field={demo} className="flex w-20 place-content-center place-items-center rounded-md border border-v-red font-flex text-v-red md:w-28 md:py-1 md:hover:bg-v-red md:hover:text-white">
                      DEMO
                    </PrismicLink>
                  )}
                  {github && (
                    <PrismicLink field={github} className="flex w-20 place-content-center place-items-center rounded-md border border-black font-flex md:w-28 md:py-1 md:hover:bg-black md:hover:text-white">
                      GitHub
                    </PrismicLink>
                  )}
                </div>
              ...
              <div className="relative hidden aspect-[5/3] md:block md:w-3/5">{featuredMedia && <Media field={featuredMedia} isCoverImage={true} />}</div>
            </div>
                ...
                <div className="md:w-1/2">
                  <div
                    className={`transform-all flex flex-col break-words transition-opacity duration-[200ms] md:!h-auto md:transform-none md:!opacity-100 ${
                      isShowText ? "visible my-8 opacity-100" : "h-0 opacity-0 md:!m-0 md:!block md:!pl-[5vw]"
                    }`}
                  >
                  ...
                  <div className={`my-4 flex place-content-center py-4 duration-[200ms] md:hidden ${isShowText ? "rotate-180" : "rotate-0"}`} onClick={() => setIsShowText(!isShowText)}>
                    <span
                      className={`
        ...
        <div className="no-scrollbar mt-16 mb-16 flex flex-nowrap gap-x-4 overflow-x-scroll bg-v-light-gray py-8 md:mb-20 md:gap-x-12 md:pt-28 md:pb-20" ref={ref}>
              ...
              isFilled.linkToMedia(project.data?.featuredMedia) && (
                <PrismicLink field={project} key={project.id} className="duration-[400ms] first:pl-4 last:pr-4 md:first:pl-[5vw] md:last:pr-[5vw] md:hover:opacity-60">
                  <div className="flex w-[50vw] shrink-0 flex-col md:w-[30vw]">
                    <Image src={project.data?.featuredMedia.url ?? ""} alt={project.data?.title ?? "PROJECT"} fill className="!relative mb-2 aspect-[5/3] w-full object-cover" />
