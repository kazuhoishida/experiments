...
import type { FeaturedProjectsDocumentWithLinks, ProjectFetchKeyUnion, CreatorFetchKeyUnion,} from "../prismic-additional"
...
export function fetchFeaturedProjects(client: Client) {
  return client.getSingle<FeaturedProjectsDocumentWithLinks<TProjectFetchKey, TCreatorFetchKey>>('featured-projects', {
    fetchLinks: [...projectFetchKeys.map(key => `project.${key}`), ...creatorFetchKeys.map(key => `creator.${key}`),],
  })
}
